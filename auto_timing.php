<?php 
	...
	//day 1
	$sql_28 = "SELECT id, nomination, duration, team_name, dance_style, fio, city, file_name, TIME_TO_SEC(duration) as sec_dur  FROM `DATA_INPUT` WHERE nomination != 'Street Show' and nomination !='Show Girls' ORDER by min_age  , skill_level";
	//day 2
	$sql_29 = 'SELECT id, nomination, duration, team_name, dance_style, fio, city, file_name, TIME_TO_SEC(duration) as sec_dur FROM `DATA_INPUT` WHERE nomination = "Street Show" or  nomination ="Show Girls" ORDER by min_age  , skill_level;' ;
...
//	fputcsv($file_open, $file_data); 
	while($day_1 = $result_28->fetch_assoc()) {
		...
		$file_data = array($day_1["id"],$day_1["duration"],
	 	...
	 	array_push($global_data, $file_data);
	 	// fputcsv($file_open, $file_data); 
		echo $counter."::".$day_1["id"]."::".$day_1["duration"]."::".$day_1["nomination"]."::".$day_1["team_name"]."::".$day_1["dance_style"]."::".$day_1["fio"]."::".$day_1["city"]."::".$day_1["file_name"]."::=SUM(B".$counter_day_2.";"."I".($counter_day_2-1)."):::";
	}
	$counter_day_2++;
	$counter = 0;
	//day 2
	...
	while($day_2 = $result_29->fetch_assoc()) {
		...
		$day_2["nomination"]."::".$day_2["team_name"]."::".$day_2["dance_style"]."::".$day_2["fio"]."::".$day_2["city"]."::".$day_2["file_name"]."::=SUM(B".$counter_day_2.";"."I".($counter_day_2-1)."):::";
	}
